...
  // Last-chance fallback for start date
  if (start == null) {
    if (typeof sub.billing_cycle_anchor === 'number') {
      start = sub.billing_cycle_anchor;
    } else if (typeof sub.start_date === 'number') {
...
  // 1. Prioritize the mirrored Stripe JSON payload in `data` as the source of truth
  if (dbSub.data) {
    try {
...
      // Fallback to items.data[0] within the payload
      if (start == null || end == null) {
...
      // Fallback start to start_date or billing_cycle_anchor from payload
      if (start == null) {
        if (typeof payload?.start_date === 'number') {
          start = payload.start_date;
        } else if (typeof payload?.billing_cycle_anchor === 'number') {
          start = payload.billing_cycle_anchor;
        }
      }

      // If we found valid dates in the payload, return them (they will be UNIX timestamps)
      if (start !== undefined || end !== undefined) {
        return { start, end };
      }
    } catch {
      // JSON parsing failed, proceed to the fallback
    }
  }

  // 2. Fallback to top-level Prisma Date fields if `data` is missing or fails to parse
...
  // 1. Prioritize the mirrored Stripe JSON payload in `data`
  if (dbSub.data) {
    try {
      const payload = typeof dbSub.data === 'string' ? JSON.parse(dbSub.data) : dbSub.data;
      let trialStart =
...
      if (trialStart !== undefined || trialEnd !== undefined) {
        return { trialStart, trialEnd };
      }
    } catch {
      // JSON parsing failed, proceed to the fallback
...
  // 2. Fallback to top-level Prisma Date fields
