...
import { Card, CardContent, CardHeader, CardTitle } from '@singr/ui'
...
import { formatAmountForDisplay } from '@singr/stripe-billing/format'
  ...
  const getPlanName = (price: StripePrice) => {
    if (price.metadata?.['plan_name']) {
      switch (price.metadata['plan_name']) {
        ...
        default: return price.productRel.name || 'Plan'
      }
    }
    return price.productRel.name || 'Plan'
  }
